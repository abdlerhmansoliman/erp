...
const props = defineProps({
  type: { type: String, required: true }, // purchase | sales | returns ...
  title: { type: String, required: true }, // "Purchase Invoice" / "Sales Invoice"
  partyLabel: { type: String, required: true }, // Supplier / Customer
  invoice: { type: Object, required: false },
  loading: { type: Boolean, default: false },
  invoiceSummary: { type: Object, required: false },
...
<template>
  <div class="p-6 max-w-7xl mx-auto">
    ...
    <div v-if="loading" class="text-center py-12">
      ...
      <p class="mt-4 text-gray-600">Loading {{ title.toLowerCase() }}...</p>
    </div>
    ...
    <div v-else-if="invoice" class="space-y-6">
      ...
      <div class="bg-white rounded-lg shadow-sm border p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <div class="bg-gray-50 p-4 rounded-lg">
            <h3 class="text-sm font-semibold text-gray-700 mb-2">{{ partyLabel }}</h3>
            <p class="text-lg font-medium text-gray-900">{{ invoice[`${type}_name`] || 'N/A' }}</p>
            <p class="text-sm text-gray-600 mt-1" v-if="invoice[`${type}_email`]">
              {{ invoice[`${type}_email`] }}
            </p>
            <p class="text-sm text-gray-600" v-if="invoice[`${type}_phone`]">
              {{ invoice[`${type}_phone`] }}
            </p>
          </div>
      ...
      <ShowInvoiceTable
        ...
        :show-discount="true"
        currency="$"
        empty-message="No items found in this invoice"
      />
    ...
    <div v-else class="text-center py-12">
      ...
      <p class="text-gray-600 mt-2">The requested invoice could not be loaded.</p>
      <button 
        @click="goBack"
