...
onMounted(async () => {
  try {
    ...
    toast.error('Failed to fetch product data or options');
    console.error(error);
  }
});
    ...
    <form @submit.prevent="submitForm" class="space-y-5">
      ...
      <div class="flex flex-col">
        ...
        <textarea v-model="product.description" placeholder="Enter product description"
          class="border border-gray-300 rounded-md p-3 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-transparent transition resize-none h-24"></textarea>
      </div>

      <!-- Price -->
      <div class="flex flex-col">
        <label class="mb-1 font-medium text-gray-700">Price</label>
        <input v-model="product.price" type="number" step="0.01" placeholder="Enter sale price"
          class="border border-gray-300 rounded-md p-3 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-transparent transition" />
      </div>
      ...
      <div class="flex flex-col">
        ...
        <input v-model="product.purchase_price" type="number" step="0.01" placeholder="Enter purchase price"
          class="border border-gray-300 rounded-md p-3 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-transparent transition" />
      </div>
